...
%
%  Версия: 2.1 (с флагами управления)
%  Дата:   2024
%  =========================================================================
...
fprintf('Pfa:              %.2e\n', cfg.Pfa);
fprintf('\n--- ФЛАГИ УПРАВЛЕНИЯ ---\n');
fprintf('Шум:        %s\n', iif(cfg.enable.NOISE, 'ВКЛ', 'ВЫКЛ'));
fprintf('ЧМП:        %s\n', iif(cfg.enable.MTI, 'ВКЛ', 'ВЫКЛ'));
fprintf('Обнаружение:%s\n', iif(cfg.enable.DETECTION, 'ВКЛ', 'ВЫКЛ'));
fprintf('Графики:    %s\n', iif(cfg.enable.PLOTS, 'ВКЛ', 'ВЫКЛ'));
fprintf('========================================\n');
...
%% =========================================================================
%  4. МОДЕЛИРОВАНИЕ СИГНАЛА (ЕСЛИ ВКЛЮЧЕН КАНАЛ)
%  =========================================================================
...
if cfg.enable.CHANNEL
    ...
    % Если канал выключен — создаем заглушку (шум или нули)
    fprintf('Канал распространения ВЫКЛЮЧЕН\n');
    rx_H_cell = cell(1, cfg.NumPulses);
    rx_V_cell = cell(1, cfg.NumPulses);
    for pulse = 1:cfg.NumPulses
        rx_H_cell{pulse} = zeros(N, 1);
        rx_V_cell{pulse} = zeros(N, 1);
    end
end

%% =========================================================================
%  5. ДОБАВЛЕНИЕ ШУМА (ЕСЛИ ВКЛЮЧЕН)
%  =========================================================================
...
elseif cfg.enable.CHANNEL
    fprintf('Шум ВЫКЛЮЧЕН\n');
else
    fprintf('Шум пропущен (канал выключен)\n');
end
...
% --- 6.1. Согласованный фильтр (если включен) ---
if cfg.enable.MATCHED_FILTER && cfg.enable.CHANNEL
    ...
    % Если фильтр выключен, используем сигнал как есть
...
% --- 6.3. Когерентное накопление (если включено) ---
...
R_y = R_y(idx);
y_H_norm = y_H_accum(idx) / max(abs(y_H_accum(idx)) + eps);
y_V_norm = y_V_accum(idx) / max(abs(y_V_accum(idx)) + eps);
...
if cfg.enable.DETECTION && cfg.enable.CHANNEL
    ...
    target_detected_H = false;
    target_detected_V = false;
    clutter_detected_H = false;
    clutter_detected_V = false;
end

%% =========================================================================
%  9. ВИЗУАЛИЗАЦИЯ (ЕСЛИ ВКЛЮЧЕНА)
%  =========================================================================
...
if cfg.enable.PLOTS && cfg.enable.CHANNEL
    ...
    % График 4: После обработки с порогом
    subplot(3,3,4);
    ...
    if cfg.enable.DOPPLER
        ...
        subplot(3,3,6);
        text(0, 0.5, 'ДОПЛЕР ВЫКЛЮЧЕН', 'FontSize', 14, 'HorizontalAlignment', 'center');
        title('Доплер');
        grid on;
        subplot(3,3,7);
        text(0, 0.5, 'ДОПЛЕР ВЫКЛЮЧЕН', 'FontSize', 14, 'HorizontalAlignment', 'center');
    ...
    if cfg.enable.MTI
        ...
        text(0, 0.5, 'ЧМП ВЫКЛЮЧЕН', 'FontSize', 14, 'HorizontalAlignment', 'center');
        title('Эффективность ЧМП');
        grid on;
    end
...
%% =========================================================================
%  11. ДОПОЛНИТЕЛЬНЫЙ ГРАФИК: АНАЛИЗ SNR (ЕСЛИ ВКЛЮЧЕН)
%  =========================================================================

if cfg.enable.SNR_ANALYSIS && cfg.enable.PLOTS && cfg.enable.CHANNEL
